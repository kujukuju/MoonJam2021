class PerfectWord extends Ability {
    static TEXTURE = PIXI.Texture.from('assets/perfect.png');

    static HIT = new Howl({src: 'assets/Sounds/osu hit.mp3', volume: 0.6 * MusicConstants.BASE});
    ...
    constructor(position) {
        ...
        this._sprite = new PIXI.Sprite(PerfectWord.TEXTURE);
    ...
    update(time, dt) {
        if (this._deltaTime > PerfectWord.EASE_IN_DURATION + PerfectWord.LIFETIME + PerfectWord.EASE_OUT_DURATION) {
            this.destroy();
            return;
        }
        ...
        const scaleProgress = Math.min(this._deltaTime / PerfectWord.SCALE_DURATION, 1);
        const scaleValue = MathHelper.bounce(scaleProgress);
        this._sprite.scale.x = scaleValue * 2;
        this._sprite.scale.y = scaleValue * 2;

        if (this._deltaTime < PerfectWord.EASE_IN_DURATION) {
            const easeInProgress = Math.min(this._deltaTime / PerfectWord.EASE_IN_DURATION, 1);
        ...
        } else if (this._deltaTime < PerfectWord.EASE_IN_DURATION + PerfectWord.LIFETIME) {
            this._sprite.alpha = 1;
            // this._sprite.scale.x = 1;
            // this._sprite.scale.y = 1;
        } else {
            const deltaTime = Math.max(this._deltaTime - (PerfectWord.EASE_IN_DURATION + PerfectWord.LIFETIME), 0);
            const easeOutProgress = Math.min(deltaTime / PerfectWord.EASE_OUT_DURATION, 1);
