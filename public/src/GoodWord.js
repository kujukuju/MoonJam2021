class GoodWord extends Ability {
    static TEXTURE = PIXI.Texture.from('assets/good.png');
    ...
    constructor(position) {
        ...
        this._sprite = new PIXI.Sprite(GoodWord.TEXTURE);
    ...
    update(time, dt) {
        if (this._deltaTime > GoodWord.EASE_IN_DURATION + GoodWord.LIFETIME + GoodWord.EASE_OUT_DURATION) {
            this.destroy();
            return;
        }
        ...
        // const scaleProgress = Math.min(this._deltaTime / GoodWord.SCALE_DURATION, 1);
        // const scaleValue = MathHelper.bounce(scaleProgress);
        // this._sprite.scale.x = scaleValue * 2;
        // this._sprite.scale.y = scaleValue * 2;

        if (this._deltaTime < GoodWord.EASE_IN_DURATION) {
            const easeInProgress = Math.min(this._deltaTime / GoodWord.EASE_IN_DURATION, 1);
        ...
        } else if (this._deltaTime < GoodWord.EASE_IN_DURATION + GoodWord.LIFETIME) {
            this._sprite.alpha = 1;
            this._sprite.scale.x = 2;
            this._sprite.scale.y = 2;
        } else {
            const deltaTime = Math.max(this._deltaTime - (GoodWord.EASE_IN_DURATION + GoodWord.LIFETIME), 0);
            const easeOutProgress = Math.min(deltaTime / GoodWord.EASE_OUT_DURATION, 1);
