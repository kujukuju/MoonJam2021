class BossBatEntity extends EnemyEntity {
    ...
    _getRelativeBeat(time) {
        return MusicManager.getCurrentEventBeat(time, 'bass3');
    }

    _getBeatDuration(time) {
        const beat = this._getRelativeBeat(time);
        return MusicManager.getEventDuration(Math.round(beat), 'bass3');
    }

    _getApproximateBeatTime(beat) {
        return MusicManager.getApproximateTimeForEvent(Math.round(beat), 'bass3');
    }

    _getNextBeatTimeAfterTime(time) {
        return MusicManager.getEventTimeAfterTime(time, 'bass3');
    }
}